...
import com.konka.videoplayer.config.CommonConfig;
 ...
 * On 2016/06/02 00:01
 */
public class KKResizeTextureView extends TextureView {
    protected static final String TAG = "JZResizeTextureView";

    public int currentVideoWidth = 0;
    public int currentVideoHeight = 0;

    public KKResizeTextureView(Context context) {
        super(context);
        currentVideoWidth = 0;
        currentVideoHeight = 0;
    }

    public KKResizeTextureView(Context context, AttributeSet attrs) {
        super(context, attrs);
        currentVideoWidth = 0;
        currentVideoHeight = 0;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (CommonConfig.VIDEO_IMAGE_DISPLAY_TYPE == CommonConfig.VIDEO_IMAGE_DISPLAY_TYPE_FILL_PARENT) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (CommonConfig.VIDEO_IMAGE_DISPLAY_TYPE == CommonConfig.VIDEO_IMAGE_DISPLAY_TYPE_ORIGINAL) {
                /**原图**/
                height = videoHeight;
                width = videoWidth;
            } else if (CommonConfig.VIDEO_IMAGE_DISPLAY_TYPE == CommonConfig.VIDEO_IMAGE_DISPLAY_TYPE_FILL_SCROP) {
                if (viewRotation == 90 || viewRotation == 270) {
                    int tempSize = parentWidth;
                    parentWidth = parentHeight;
                    parentHeight = tempSize;
